...
let mkarchive dir =
  ...
  in
  let module Archiver = Belenios_core.Archive.MakeArchiver (IoArchiver) (Tool_events.Writer) in
...
let cmd =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P "This command reads files from an extracted $(i,UUID.bel) and outputs (on standard output) a canonical archive containing the same files.";
    ] @ common_man
  in
